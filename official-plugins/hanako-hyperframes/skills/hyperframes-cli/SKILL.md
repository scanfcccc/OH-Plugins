...
Before running or recommending any HyperFrames CLI command, first check that the HyperFrames CLI is available in the user's environment; if it is missing, tell the user to install or configure it instead of assuming it exists.

Everything runs through `npx hyperframes`. Requires Node.js >= 22 and FFmpeg.

## Dependency Checks

Before the first HyperFrames CLI operation in a workspace, check the required runtime dependencies:

- `node --version` must report Node.js 22 or newer.
- `npx --version` must be available, because the default command is `npx hyperframes`.
- `ffmpeg -version` and `ffprobe -version` must be available for media inspection and rendering.
- `npx hyperframes --version` or the configured HyperFrames command must run successfully.

If a project-local package install is needed, the agent may run the normal package-manager install for that project after explaining it. For system-level dependencies such as Node.js, FFmpeg, Docker, Homebrew, winget, or Chocolatey, explain what is missing and ask the user before installing or changing the machine.
